/**
 * useMcp React hook copied from use-mcp repo
 * Why? Because it isn't always updated, so I 
 * want to make sure if I adopt it that it can
 * be updated.
 * {@see https://github.com/modelcontextprotocol/use-mcp}
 */
import {
  ...
  const connect = useCallback(async () => {
    ...
    const tryConnectWithTransport = async (transportType: TransportType): Promise<'success' | 'fallback' | 'auth_redirect' | 'failed'> => {
      ...
      try {
        ...
        const targetUrl = new URL(decodeURIComponent(sanitizedUrl));
